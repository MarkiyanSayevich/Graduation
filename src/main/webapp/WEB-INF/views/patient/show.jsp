...
<body>
	...
	<c:url var="firstUrl"
		value="/patient/filter?page=0&name=${ filter.name }&size=${ sizeModel }" />
	<c:url var="lastUrl"
		value="/patient/filter?page=${ page.totalPages - 1 }&name=${ filter.name }&size=${ sizeModel }" />

	<c:url var="nextUrl"
		value="/patient/filter?page=${ currentPage + 1 }&name=${ filter.name }&size=${ sizeModel }" />
	<c:url var="prevUrl"
		value="/patient/filter?page=${ currentPage - 1 }&name=${ filter.name }&size=${ sizeModel }" />
	...
	<div class="btn-toolbar" style="margin-bottom: 30px; margin-left: 10px">
		<a href="/patient/add" class="btn btn-primary"><span
			class="glyphicon glyphicon-plus"></span> Додати</a>
	</div>

	<div class="alert alert-info">
		<strong>Список пацієнтів ... </strong>
	</div>
	...
	<table class="table table-hover">
		...
		<tbody>
			<c:forEach items="${ patientModel }" var="patient">
				<tr>
					<td>${ patient.id }</td>
